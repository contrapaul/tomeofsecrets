...
describe('events', () => {
  ...
  it('a card pick pauses the script and resumes after the answer', () => {
    ...
    let st = startDialogue(run, content, 'binding-ritual')!;
    st = advance(run, content)!;
    expect(st.waiting).toBe('choice');
    ...
    st = pick(run, content, [run.hero.deck[0]!.uid])!;
  ...
  it('a fight from a script returns to the script with a doubled reward', () => {
    ...
    let st = startDialogue(run, content, 'mimic-shelf')!;
    st = advance(run, content)!;
    st = advance(run, content)!;
    expect(st.waiting).toBe('choice');
    st = choose(run, content, 0)!;
